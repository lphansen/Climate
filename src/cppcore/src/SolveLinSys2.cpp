...
stateVars::stateVars (Eigen::MatrixXd preLoad) {
    ...
    N = preLoad.cols();  // # dimensions, 3
    S = preLoad.rows();  // # grid points in total
    stateMat.resize(S,N);
    dVec.resize(N); dVec.setZero(); // 3 那个
    increVec.resize(N); increVec.setZero(); // 3
    ...
    for (int i = 1; i < S; ++i) {  
        for (int n = 0; n < N; ++n ) {
            ...
            if (diff > 0 && dVec(n) == 0 && increVec(n) == 0) {
                dVec(n) = diff; // dim 1 ~ 1; dim 2 ~ 30; dim 3 ~ 1200
                increVec(n) = i;
            }
        }
