...
*/
    ...
    // std::cout << "Sparse matrix size, row: "<< Le.rows()
    //             << ", col:" << Le.cols() << std::endl;
    // ofstream myfile;
    // myfile.open("test.data");
    // for (const auto& triplet: matList){
    //     // std::cout << "triplet, row: " << triplet.row()
    //     //     << ", col:" << triplet.col() << ", value:" << triplet.value() <<
    //     //     std::endl;
    //     myfile << triplet.row() << "," << triplet.col() << ',' << triplet.value() << endl;
    // }
...
Eigen::MatrixXd test1(py::array_t<double>& A){
    py::buffer_info buf = A.request();
    // printf("S: %3i\n", buf.size);
    Eigen::Map<Eigen::MatrixXd> A1((double *)buf.ptr, buf.shape[0], buf.shape[1]);
    return A1;
}


// stateVars test2(stateVars stateSpace){
//     return stateSpace * 2;
// }

double test3(double dt){
    return dt * 2;
}

class model{
public:
    MatrixXdR preLoadMat;
    MatrixXdR A;
    MatrixXdR B;
    MatrixXdR C;
    MatrixXdR D;
    ...
    model(MatrixXdR preLoadMat_input, MatrixXdR A_input, MatrixXdR B_input, MatrixXdR C_input, MatrixXdR D_input, MatrixXdR v0_input, double dt_input)
    {
        preLoadMat = preLoadMat_input;
        A = A_input;
        B = B_input;
        C = C_input;
        D = D_input;
...
py::tuple solvels(Eigen::Ref<MatrixXdR> preLoadMat, Eigen::Ref<MatrixXdR> A, Eigen::Ref<MatrixXdR> B, Eigen::Ref<MatrixXdR> C,  Eigen::Ref<MatrixXdR> D, Eigen::Ref<MatrixXdR> v0, double dt)
{
    // std::cout << "Mapping data to EIGEN:" << std::endl;
    py::tuple data(3);
    // tic();
...
PYBIND11_MODULE(SolveLinSys1,m){
    ...
    m.def("solvels", &solvels, py::arg("stateSpace"),
        py::arg("A"), py::arg("B"), py::arg("C"), py::arg("D"),
        py::arg("v0"), py::arg("dt"));

    m.def("test1", &test1);
    // m.def('test2', &test2, py::arg("stateSpace"));
    m.def("test3", &test3);

    py::class_<model>(m,"model")
    .def(py::init<MatrixXdR, MatrixXdR, MatrixXdR, MatrixXdR, MatrixXdR, MatrixXdR, double>())
    .def("solvemodel", &model::solvemodel)
    .def("getresult", &model::getresult);
}