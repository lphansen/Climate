...
tol = 1e-16; % tol
dt = 0.5; % epsilon
v0 = (kappa).*r_mat+(1-kappa).*k_mat-beta_f.*t_mat; % initial guess
...
% FOC
B1 = v0_dr-xi_d.*(gamma_1(1)+gamma_2(1)*(t_mat).*beta_f+gamma_2_plus(1).*(t_mat.*beta_f-gamma_bar).^(power-1).*(t_mat>=(crit./beta_f))) ...
...
scale_2_fnc = @(x) exp(-1./xi_p.*xi_d.*(gamma_1.*x ...
    +gamma_2.*x.^2.*t_mat ...
    +gamma_2_plus.*x.*(x.*t_mat-gamma_bar).^(power-1).*((x.*t_mat-gamma_bar)>=0)).*exp(r_mat).*e_hat) ...
...
q2_tilde_fnc = @(x) exp(-1./xi_p.*xi_d.*(gamma_1.*x ...
    +gamma_2.*x.^2.*t_mat ...
    +gamma_2_plus.*x.*(x.*t_mat-gamma_bar).^(power-1).*((x.*t_mat-gamma_bar)>=0)).*exp(r_mat).*e_hat)./scale_2;
I_2 = -1.*xi_p.*log(scale_2);
J_2_without_e_fnc = @(x) xi_d.*exp(r_mat).*...
    q2_tilde_fnc(x) ...
    .*(gamma_1.*x +gamma_2.*t_mat.*x.^2 ...
    +gamma_2_plus.*x.*(x.*t_mat-gamma_bar).^(power-1).*((x.*t_mat-gamma_bar)>=0)) ...
    .*normpdf(x,beta_f,sqrt(var_beta_f));
...
while (max(max(max(abs(out_comp - vold))))) > tol % check for convergence
   ...
   vold = v0 .* ones(size(v0));
    ...
    while Converged == 0
        ...
        jstar = (q.*exp(psi_1.*(r_mat-k_mat))./((v0_dr).*psi_1.*psi_1)).^(1./(psi_1 - 1));
...
if weight == 1
    jfunc = griddedInterpolant(r_mat,t_mat,k_mat,j,'spline');
else
    jfunc = griddedInterpolant(r_mat,t_mat,k_mat,j,'linear');
end
...
base_model_drift_func = @(x) exp(r_mat).*e...
        .*(gamma_1.*x +gamma_2.*t_mat.*x.^2 ...
        +bar_gamma_2_plus.*x.*(x.*t_mat-gamma_bar).^(power-1).*((x.*t_mat-gamma_bar)>=0))...
        .*normpdf(x,beta_f,sqrt(var_beta_f)); ...
...
Model2 = load(file2,'r_mat','k_mat','t_mat','i_k','j','e','alpha','kappa','delta','xi_p');
r_mat = Model2.r_mat;
k_mat = Model2.k_mat;
t_mat = Model2.t_mat;
xi_p = Model2.xi_p;
