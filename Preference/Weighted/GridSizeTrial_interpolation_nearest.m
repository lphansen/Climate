...
% % addpath('/home/hanxuh/Desktop/Climate_test_server')
% 
...
% end
% 
% s0 = '/HJB_NonLinPref_Cumu_03_4xF';
% filename = [pwd, s0];
% save(filename); % save HJB solution
% stop;

%% Step 2.5 interpolation
filename3 = [pwd, s0, '_Sims'];
...
% save results
save('HJB_NonLinPref_Cumu_grid_range_1_Sims', 'e_hists2', 'hists2', 'i_k_hists2', 'j_hists2', 'psi_1', ...
'v_dk_hists2', 'v_dr_hists2', 'v_dt_hists2');
e_values = mean(e_hists2,2);
s1 = num2str(1.*xi_p,4);
s1 = strrep(s1,'.','');
s_e = strcat('e_A=',s1,'_old','.mat');
save(s_e,'e_values');
...
l1 = "K = 18, F = 4000, R = 9";
l2 = "K = 13.5, F = 6000, R = 13.5";

psi_1 = 0.1429;
% new = load(['HJB_NonLinPref_Cumu_grid_range_', num2str(run), '_Sims']);
% stop
% old = load("HJB_NonLinPref_F6000_R135_K135_4xF_2xRK_8_10_03_Sims");
new = load('Sims_averse_4500');
old = load(['HJB_NonLinPref_Cumu_grid_range_', num2str(run), '_Sims']);
newe = load('e_A=00002222_new');
olde = load('e_A=00002222_old');

figure('Position', [1 1 800 600])
plot(new.v_dr_hists2, 'LineWidth', 3)
hold on;
plot(old.v_dr_hists2, 'LineWidth', 3)
legend(l1,l2);
title("v_r, 4xF, 2xR, 2xK")
...
figure('Position', [1 1 800 600])
plot(new.j_hists2, 'LineWidth', 3)
hold on;
plot(old.j_hists2, 'LineWidth', 3)
legend(l1,l2);
title("j, 4xF, 2xR, 2xK")

figure('Position', [1 1 800 600])
plot(psi_0 * (new.j_hists2 .* new.hists2(:,2) ./ new.hists2(:,1)).^psi_1, 'LineWidth', 3)
hold on;
plot(newe.e_values ./ new.hists2(:,1), 'LineWidth', 3)
plot(psi_0 * (old.j_hists2 .* old.hists2(:,2) ./ old.hists2(:,1)).^psi_1, 'LineWidth', 3)
plot(olde.e_values ./ old.hists2(:,1), 'LineWidth', 3)
legend({'$\psi_0(\frac{J}{R})^{\psi_1}$ new','$(\frac{E}{R})$ new','$\psi_0(\frac{J}{R})^{\psi_1}$ old','$(\frac{E}{R})$ old'} , 'Interpreter', 'latex', 'FontSize', 20);
title("J/R, 4xF, 2xR, 2xK")

figure('Position', [1 1 800 600])
plot(new.j_hists2.^psi_1, 'LineWidth', 3)
hold on;
plot(old.j_hists2.^psi_1, 'LineWidth', 3)
legend(l1,l2);
title("j contribution to drift, 4xF, 2xR, 2xK")

figure('Position', [1 1 800 600])
plot(log(new.hists2(:,1)), 'LineWidth', 3)
hold on;
plot(log(old.hists2(:,1)), 'LineWidth', 3)
legend(l1,l2);
title("r, 4xF, 2xR, 2xK")

figure('Position', [1 1 800 600])
plot(log(new.hists2(:,2)), 'LineWidth', 3)
hold on;
plot(log(old.hists2(:,2)), 'LineWidth', 3)
legend(l1,l2);
title("k, 4xF, 2xR, 2xK")

figure('Position', [1 1 800 600])
plot(new.hists2(:,3), 'LineWidth', 3)
hold on;
plot(old.hists2(:,3), 'LineWidth', 3)
legend(l1,l2);
title("f, 4xF, 2xR, 2xK")

figure('Position', [1 1 800 600])
plot(newe.e_values, 'LineWidth', 3)
hold on;
plot(olde.e_values, 'LineWidth', 3)
legend(l1,l2);
title("emissions, 4xF, 2xR, 2xK")
...
% 
% 
% RE_func = griddedInterpolant(r_mat,F_mat,k_mat,RE,'spline');
% pi_tilde_1_func = griddedInterpolant(r_mat,F_mat,k_mat,pi_tilde_1,'spline');
% pi_tilde_2_func = griddedInterpolant(r_mat,F_mat,k_mat,pi_tilde_2,'spline');
