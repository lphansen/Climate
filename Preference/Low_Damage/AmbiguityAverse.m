...
I_term = -xi_p.*log(pi_tilde_1+pi_tilde_2);
R_1 = 1./xi_p.*(I_1 - J_1);     
R_2 = 1./xi_p.*(I_2 - J_2);
...
% load results
file1 = [pwd, '/HJB_NonLinPref_Cumu'];
Model1 = load(file1,'R_1','pi_tilde_1_norm','r_mat','k_mat','F_mat',...
    'beta_tilde_1','beta_f','lambda_tilde_1','xi_p','var_beta_f'); 
...
RE_1_func = griddedInterpolant(r_mat,F_mat,k_mat,RE_1,'spline');
...
for time=1:400
    RE_1_plot(time) = RE_1_func(log(R_value(time,1)),F_value(time,1),log(K_value(time,1)));
    weight_plot(time) = pi_tilde_1_func(log(R_value(time,1)),F_value(time,1),log(K_value(time,1)));
    nordhaus_mean(time) = beta_tilde_1_func(log(R_value(time,1)),F_value(time,1),log(K_value(time,1)));
    sd_nordhaus(time) = 1./sqrt(lambda_tilde_1_func(log(R_value(time,1)),F_value(time,1),log(K_value(time,1))));
end
