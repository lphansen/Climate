...
load([directory, '/HJB_NonLinGrowth_NoAmb']);
% load([directory, '/HJB_NonLinGrowth_NoAmb']);
...
out = v0;
    ...
    B_k = Alpha+Gamma.*log(1+i_k./Theta)-0.5.*(sigma_o.^2)-Gamma_base;
...
directory = pwd;
% filename2 = [directory, '/SCC_mat_Cumu_base_GrowthNoAmb'];
filename2 = [directory, '/SCC_mat_Cumu_base_GrowthAmb_v2b'];
...
out = v0;
    ...
    A = -delta.*ones(size(r_mat_1));
    B_r = -e_star+Gamma_r.*(f.^Theta_r)-0.5.*(sigma_r.^2);
    B_k = Alpha+Gamma.*log(1+i_k./Theta)-0.5.*(sigma_o.^2)-Gamma_tilted;
    B_t = e_star.*exp(r_mat_1);
    C_rr = 0.5.*sigma_r.^2.*ones(size(r_mat_1));
    C_kk = 0.5.*sigma_k.^2.*ones(size(r_mat_1));
    C_tt = zeros(size(r_mat));
...
directory = pwd;
% filename2 = [directory, '/SCC_mat_Cumu_worst_GrowthNoAmb'];
filename2 = [directory, '/SCC_mat_Cumu_worst_GrowthAmb_v2b'];
